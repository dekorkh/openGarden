...
namespace OpenGarden
{
    class Level_Plant : Level
    {
        public override void LoadLevel(GameAudio GA)
        {
            ...
            Vector3 dynbox_target_Pos = new Vector3(0f, 0f, -4f);
            float boxsize = .9f;
            float targetsize = .3f;
            ...
            DynBox dynbox = new DynBox(new Vector3(0f, 1f, 0f));
            dynbox.Position = dynbox_Pos;
            dynbox.Scale = new Vector3(boxsize);
            dynbox.Target = new Vector3(-1f, -1f, -1f);

            DynBox dynbox_target = new DynBox(new Vector3(0f, 0f, 1f));
            dynbox_target.Position = dynbox_target_Pos;
            dynbox_target.Scale = new Vector3(targetsize);

            dynbox.animlist.Add(new Anim_Orbit(dynbox_target, 120f, new Vector3(0f, 0f, -4f), 1.5f, ding));
        ...
        public override void Tick()
        {
            ...
            scene[1].Target = scene[0].Position;
            ((Line)scene[2]).End = ((DynBox)scene[0]).X_Axis * .2f;
            ((Line)scene[3]).End = ((DynBox)scene[0]).Y_Axis * .2f;
            ((Line)scene[4]).End = ((DynBox)scene[0]).Z_Axis * .2f;
        }
    }
}
